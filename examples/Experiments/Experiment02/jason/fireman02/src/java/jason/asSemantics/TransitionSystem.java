...
public class TransitionSystem {
	...
	public Boolean blockedRealWorldPerceptions = true;
	public Boolean blockedRFMessages = true;
		...
		if (conf.C.hasMsg()) {
			...
			if (m.getPropCont() instanceof Term) {
				...
				try {
					...
					logger.warning("The content of the message '"
							+ m.getPropCont() + "' is not a term!");
					return;
				}
			}
			...
			if (intention != null) {
				...
				Structure send = (Structure) intention.peek().getCurrentStep()
						.getBodyTerm();
				...
				} else if (send.getTerm(1).toString().equals("askAll")
						&& content.isList()) { // adds source in each answer if
												// possible
				...
				if (rec.isList()) { // send to many receivers
					...
					VarTerm answers = new VarTerm("AnsList___"
							+ m.getInReplyTo());
					...
					if (listOfAnswers.size() == nbReceivers) { // all agents
																// have answered
						resumeSyncAskIntention(m.getInReplyTo(),
								send.getTerm(3), listOfAnswers);
					}
				} else {
					resumeSyncAskIntention(m.getInReplyTo(), send.getTerm(3),
							content);
				}
				...
				if (!m.isReplyToSyncAsk()) { // ignore answer after the timeout
					...
					boolean added = false;
					if (!setts.isSync()
							&& !ag.getPL().hasUserKqmlReceivedPlans()
							&& content.isLiteral() && !content.isList()) { // optimisation
																			// to
																			// jump
																			// kqmlPlans
						if (m.getIlForce().equals("achieve")) {
							content = add_nested_source.addAnnotToList(content,
									new Atom(sender));
							C.addEvent(new Event(new Trigger(TEOperator.add,
									TEType.achieve, (Literal) content),
									Intention.EmptyInt));
							added = true;
					...
					if (!added) {
						Literal received = new LiteralImpl(
								Message.kqmlReceivedFunctor).addTerms(new Atom(
								sender), new Atom(m.getIlForce()), content,
								new Atom(m.getMsgId()));

						updateEvents(new Event(new Trigger(TEOperator.add,
								TEType.achieve, received), Intention.EmptyInt));
					}
				} else {
					logger.fine("Ignoring message " + m
							+ " because it is received after the timeout.");
	...
	private void resumeSyncAskIntention(String msgId, Term answerVar,
			Term answerValue) throws JasonException {
			...
			generateGoalDeletion(
					...
					JasonException
							.createBasicErrorAnnots(
									"ask_failed",
									"reply of an ask message ('"
											+ answerValue
											+ "') does not unify with fourth argument of .send ('"
											+ answerVar + "')"));
		}
	...
	private void applyRelApplPlRule2(String m) throws JasonException {
		...
		if (conf.C.SE.trigger.isGoal() && !conf.C.SE.trigger.isMetaEvent()) {
			...
			String msg = "Found a goal for which there is no " + m + " plan:"
					+ conf.C.SE;
			if (!generateGoalDeletionFromEvent(JasonException
					.createBasicErrorAnnots("no_" + m, msg)))
				logger.warning(msg);
			...
			joinRenamedVarsIntoIntentionUnifier(confP.C.SI.peek(),
					confP.C.SI.peek().unif);
			updateIntention();
	...
	private void applySelAppl() throws JasonException {
		...
		if (confP.C.SO != null) {
			...
			if (logger.isLoggable(Level.FINE))
				logger.fine("Selected option " + confP.C.SO + " for event "
						+ confP.C.SE);
		} else {
			logger.fine("** selectOption returned null!");
			generateGoalDeletionFromEvent(JasonException
					.createBasicErrorAnnots("no_option",
							"selectOption returned null"));
	...
	private void applyFindOp() throws JasonException {
		...
		List<Plan> candidateRPs = conf.ag.pl
				.getCandidatePlans(conf.C.SE.trigger);
			...
			for (Plan pl : candidateRPs) {
				...
				if (relUn != null) { // is relevant
					...
					if (context == null) { // context is true
						...
						Iterator<Unifier> r = context.logicalConsequence(ag,
								relUn);
						if (r != null && r.hasNext()) {
		...
		if (conf.C.SE.intention == Intention.EmptyInt) {
			...
			if (setts.isTROon()) {
				IntendedMeans top = confP.C.SE.intention.peek(); // top = the IM
																	...
																	// TRO
				if (top != null
						&& top.getTrigger().isGoal()
						&& im.getTrigger().isGoal()
						&& // are both goal
						top.getCurrentStep().getBodyNext() == null
						&& // the plan below is finished
						top.getTrigger()
								.getPredicateIndicator()
								.equals(im.getTrigger().getPredicateIndicator()) // goals
																					// are
																					// equals
				) {
					...
					if (imBase != null) {
						...
						for (VarTerm v : imBase.renamedVars) {
							VarTerm vvl = (VarTerm) imBase.renamedVars.function
									.get(v);
							...
							if (t != null) { // if v has got a value in top
								...
								VarTerm v0 = (VarTerm) top.renamedVars.function
										.get(vvl);
								if (v0 != null) {
			...
			if (a != null) {
				...
				if (C.removePendingAction(confP.C.SI.getId()) != null) {
					if (a.getResult()) {
						...
						if (reason == null)
							reason = "";
						ListTerm annots = JasonException
								.createBasicErrorAnnots("action_failed", reason);
	...
	private void applyExecInt() throws JasonException {
		...
		if (bTerm instanceof VarTerm) { // de-var bTerm
			...
			if (bTerm.isVar()) { // the case of !A with A not ground
				String msg = h.getSrcInfo() + ": " + "Variable '" + bTerm
						+ "' must be ground.";
				if (!generateGoalDeletion(conf.C.SI,
						JasonException.createBasicErrorAnnots(
			...
			if (bTerm.isPlanBody()) {
				if (h.getBodyType() != BodyType.action) { // the case of ...; A
															// = { !g }; +g;
															// ....
					String msg = h.getSrcInfo() + ": " + "The operator '"
							+ h.getBodyType() + "' is lost with the variable '"
							+ bTerm + "' unified with a plan body. ";
					if (!generateGoalDeletion(conf.C.SI,
							JasonException.createBasicErrorAnnots(
									"body_var_with_op", msg)))
						logger.log(Level.SEVERE, msg);
					return;
				}
				h = (PlanBody) bTerm;
			...
			try {
				...
				if (oresult != null) {
					...
					if (!ok) { // IA returned false
						errorAnnots = JasonException.createBasicErrorAnnots(
								"ia_failed", "");
					}
				}
				...
				if (!generateGoalDeletion(conf.C.SI, errorAnnots))
					logger.log(Level.SEVERE,
							body.getErrorMsg() + ": " + e.getMessage());
				ok = true; // just to not generate the event again
			} catch (Exception e) {
				if (body == null)
					logger.log(Level.SEVERE,
							"Selected an intention with null body in '" + h
									+ "' and IM " + im, e);
				else
					logger.log(Level.SEVERE,
							body.getErrorMsg() + ": " + e.getMessage(), e);
			}
			if (!ok)
		...
		case constraint:
			Iterator<Unifier> iu = ((LogicalFormula) bTerm).logicalConsequence(
					ag, u);
			if (iu.hasNext()) {
				im.unif = iu.next();
				updateIntention();
			} else {
				String msg = "Constraint " + h + " was not satisfied ("
						+ h.getSrcInfo() + ").";
				generateGoalDeletion(conf.C.SI,
						JasonException.createBasicErrorAnnots(new Atom(
								"constraint_failed"), msg));
				logger.fine(msg);
			}
			break;
			...
			if (conf.ag.believes(f, u)) {
				...
				if (f.isLiteral() && !(f instanceof BinaryStructure)) {
					...
					if (body.isLiteral()) { // in case body is a var with
											...
											// instanceof Literal)
						Trigger te = new Trigger(TEOperator.add, TEType.test,
								body);
						evt = new Event(te, conf.C.SI);
						if (ag.getPL().hasCandidatePlan(te)) {
							if (logger.isLoggable(Level.FINE))
								logger.fine("Test Goal '"
										+ h
										+ "' failed as simple query. Generating internal event for it: "
										+ te);
							conf.C.addEvent(evt);
							confP.step = State.StartRC;
							fail = false;
				...
				if (fail) {
					if (logger.isLoggable(Level.FINE))
						logger.fine("Test '" + h + "' failed ("
								+ h.getSrcInfo() + ").");
					generateGoalDeletion(conf.C.SI,
							JasonException.createBasicErrorAnnots(
									"test_goal_failed", "Failed to test '" + h
											+ "'"));
				}
			}
			break;
		...
		case delAddBel:
			...
			try {
				...
				generateGoalDeletion(conf.C.SI,
						JasonException.createBasicErrorAnnots(
								"belief_revision_failed", "BRF failed for '"
										+ body + "'"));
				break;
			}
		...
		case addBelNewFocus:
			...
			boolean isSameFocus = setts.sameFocus()
					&& h.getBodyType() != BodyType.addBelNewFocus;
			if (isSameFocus) {
				newfocus = conf.C.SI;
				...
				for (VarTerm v : renamedVars) {
					...
					if (t != null) {
						...
						try {
							...
							logger.log(Level.SEVERE,
									"*** Error adding var into renamed vars. var="
											+ v + ", value=" + t + ".", e);
						}
					}
				}
	...
	public void applyClrInt(Intention i) throws JasonException {
		while (true) { // quit the method by return
			...
			if (logger.isLoggable(Level.FINE))
				logger.fine("Returning from IM " + topIM.getPlan().getLabel()
						+ ", te=" + topIM.getPlan().getTrigger() + " unif="
						+ topIM.unif);

			// produce ^!g[state(finished)] event
			if (topTrigger.getOperator() == TEOperator.add
					&& topTrigger.isGoal()) {
				if (hasGoalListener())
			...
			if (im.getTrigger().isGoal() && !im.getTrigger().isAddition()
				...
				im = i.peek();
				if (im.isFinished()
						|| !(im.unif.unifies(im.getCurrentStep().getBodyTerm(),
								topLiteral) && im.getCurrentStep()
								.getBodyType() == BodyType.achieve)
						|| im.getCurrentStep().getBodyTerm() instanceof VarTerm) {
					im = i.pop(); // +!c above
				}
				while (!i.isFinished()
						&& !(im.unif.unifies(im.getTrigger().getLiteral(),
								topLiteral) && im.getTrigger().isGoal())
						&& !(im.unif.unifies(im.getCurrentStep().getBodyTerm(),
								topLiteral) && im.getCurrentStep()
								.getBodyType() == BodyType.achieve)) {
					im = i.pop();
	...
	private void joinRenamedVarsIntoIntentionUnifier(IntendedMeans im,
			Unifier values) {
	...
	public void updateEvents(Event e) {
		...
		if (e.isInternal() || C.hasListener()
				|| ag.getPL().hasCandidatePlan(e.trigger)) {
			C.addEvent(e);
			if (logger.isLoggable(Level.FINE))
	...
	public boolean generateGoalDeletion(Intention i, List<Term> failAnnots)
		...
		if (im.isGoalAdd()) {
			...
			if (failEventIsRelevant) {
				...
				if (logger.isLoggable(Level.FINE))
					logger.fine("Generating goal deletion "
							+ failEvent.getTrigger() + " from goal: "
							+ im.getTrigger());
			} else {
				logger.warning("No failure event was generated for "
						+ failEvent.getTrigger());
			}
		}
		...
		// it is simply not going back to 'I' nor anywhere else!
			...
			logger.warning("Could not finish intention: " + i + "\tTrigger: "
					+ failEvent.getTrigger());
		}
	...
	private boolean generateGoalDeletionFromEvent(List<Term> failAnnots)
			throws JasonException {
		Event ev = conf.C.SE;
		if (ev == null) {
			logger.warning("** It was impossible to generate a goal deletion event because SE is null! "
					+ conf.C);
			return false;
		}
		...
		if (tevent.isAddition() && tevent.isGoal()) {
			...
			if (failEvent != null) {
				setDefaultFailureAnnots(failEvent, tevent.getLiteral(),
						failAnnots);
				confP.C.addEvent(failEvent);
				...
				logger.warning("No fail event was generated for "
						+ ev.getTrigger());
			}
	...
	public Event findEventForFailure(Intention i, Trigger tevent) {
		Trigger failTrigger = new Trigger(TEOperator.del, tevent.getType(),
				tevent.getLiteral());
		if (i != Intention.EmptyInt) {
			Iterator<IntendedMeans> ii = i.iterator();
			while (!getAg().getPL().hasCandidatePlan(failTrigger)
					&& ii.hasNext()) {
				...
				failTrigger = new Trigger(TEOperator.del, tevent.getType(),
						tevent.getLiteral());
			}
		}
		// if some failure handling plan is found
		if (tevent.isGoal() && tevent.isAddition()
				&& getAg().getPL().hasCandidatePlan(failTrigger)) {
	 ...
	 */
	private static void setDefaultFailureAnnots(Event failEvent, Term body,
			List<Term> failAnnots) {
		// add default failure annots
		if (failAnnots == null)
			failAnnots = JasonException.createBasicErrorAnnots(
		...
		if (body != null && body instanceof Literal) {
			...
			if (bodyterm.getSrcInfo() != null) {
				if (bodyterm.getSrcInfo().getSrcFile() != null)
					codesrc = new StringTermImpl(bodyterm.getSrcInfo()
							.getSrcFile());
				codeline = new NumberTermImpl(bodyterm.getSrcInfo()
						.getSrcLine());
			}
		}
		...
		if (eventLiteral.getAnnot("code_line") == null)
			eventLiteral.addAnnot(ASSyntax.createStructure("code_line",
					codeline));
	}
				...
				runAtBeginOfNextCycle(new Runnable() {
					public void run() {
						...
						if (intention == null) { // deadline in !!g, test if the
							...
							drop = desire.allDesires(C, body, new Unifier())
									.hasNext();
						} else if (intention.size() >= isize
								&& intention.hasTrigger(evt.getTrigger(),
										new Unifier())) {
							drop = true;
						}
						if (drop) {
							try {
								FailWithDeadline ia = new FailWithDeadline(
										intention, evt.getTrigger());
								ia.drop(TransitionSystem.this, body,
										new Unifier());
							} catch (Exception e) {
								e.printStackTrace();
							}
	...
	class FailWithDeadline extends fail_goal {
		 ...
		 */
		@Override
		public int dropIntention(Intention i, Trigger g, TransitionSystem ts,
				Unifier un) throws JasonException {
			if (i != null) {
				...
				if (i.dropGoal(g, un)) {
					...
					if (failEvent != null) {
						...
						ts.getLogger()
								.fine("'hard_deadline("
										+ g
										+ ")' is generating a goal deletion event: "
										+ failEvent.getTrigger());
						return 2;
						...
						ts.getLogger()
								.fine("'hard_deadline("
										+ g
										+ ")' is removing the intention without event:\n"
										+ i);
						return 3;
					}
				}
	...
	public List<Literal> realWorldPerceptions() {
		...
		if (((perceiving - lastPerceived) < this.limit) || this.blockedRealWorldPerceptions) {
		...
		// if (hasMsg) {
		//System.out.println("Antes de perceber!");
		if (this.agArch.getArgo().requestData(this.agArch.getPort(),
				"getPercepts")) {
	...
	public boolean reasoningCycle() {
		...
		try {
			...
			if (nrcslbr >= setts.nrcbp()) {
				...
				} else if (this.agArch.getDiffuserAgArch() != null && !this.blockedRFMessages) {
					this.agArch.addMessageToC();
				}
			}
			...
			if (canSleep()) {
				...
				if (!sleepingEvt
						&& ag.pl.getCandidatePlans(PlanLibrary.TE_JAG_AWAKING) != null) {
			...
			logger.log(Level.SEVERE, "*** ERROR in the transition system. "
					+ conf.C + "\nCreating a new C!", e);
			conf.C.create();
		}
