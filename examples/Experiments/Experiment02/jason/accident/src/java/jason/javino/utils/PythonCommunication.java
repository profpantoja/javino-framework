...
import jason.javino.enums.OperationalSystem;
...
public class PythonCommunication {
    ...
    private static void createPythonFile() {
        try {
            ...
            saveFile.printf("try:\n");
            saveFile.printf("\tcomm = serial.Serial(PORT, '9600', timeout=.1)\n");
            saveFile.printf("\tcomm.open\n");
            saveFile.printf("\tcomm.isOpen\n");
            saveFile.printf("\tif(OP=='command' or OP=='send'):\n");
            saveFile.printf("\t\tcomm.write(bytes(MSG, 'utf-8'))\n");
