	...
	protected void checkArguments(Term[] args) throws JasonException {
		...
		if (!args[0].isAtom())
			throw JasonException
					.createWrongArgument(
							this,
							"The javino illocutionary force parameter ('"
									+ args[0]
									+ "') must be an atom (front, back, left, right or stop)!");
		if (!args[0].toString().equals("front")
				&& !args[0].toString().equals("left")
				&& !args[0].toString().equals("back")
				&& !args[0].toString().equals("right")
				&& !args[0].toString().equals("stop")) {
			throw JasonException.createWrongArgument(this, "There is no '"
