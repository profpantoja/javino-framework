...
public class AgArch implements AgArchInfraTier, Comparable<AgArch> {
	...
	public void createCustomArchs(List<String> archs) throws Exception {
		...
		for (String agArchClass : archs) {
			// user custom arch
			if (!agArchClass.equals(AgArch.class.getName())
					&& !agArchClass.equals(CentralisedAgArch.class.getName())) {
				try {
					AgArch a = (AgArch) Class.forName(agArchClass)
							.newInstance();
					...
					System.out
							.println("Error creating custom agent aarchitecture."
									+ e);
					e.printStackTrace();
					ts.getLogger().log(Level.SEVERE,
							"Error creating custom agent aarchitecture.", e);
				}
			}
		}
	}
	 ...
	 */
	// remove feedback parameter and use getTS().addFeedbackAction in a
